...
import { 
  ...
  FaCalculator, FaCheckCircle, FaHistory, FaSearch,
  FaHammer, FaPaintRoller, FaBroom, FaFileSignature,
  FaPaperPlane, FaTimes, FaEye, FaEdit
} from "react-icons/fa";
...
const InvoicingPage = () => {
  ...
  const [invoices, setInvoices] = useState([
    { 
        ...
        items: [{ name: "Wall Paint", price: 450, qty: 10 }] // Mock data details
    },
    { 
  ...
  const addToCart = (item) => {
    // Generate a unique ID so we can have duplicate items with different prices if needed, 
    // or just match by name. Matching by name + price is safer for grouping.
  ...
  // EDIT PRICE IN CART (New Feature)
  ...
  const deleteInvoice = (id) => {
    if(confirm("Permanently delete this invoice?")) {
        ...
        if(viewingInvoice?.id === id) setViewingInvoice(null); // Close modal if open
    }
  };
    ...
    <div className="min-h-screen bg-[#F1F5F9]">
      ...
      <main className={`transition-all duration-300 ${isOpen ? "ml-60" : "ml-20"} p-8`}>
        ...
        {activeTab === 'builder' && (
            ...
            <div className="lg:col-span-2 space-y-6">
                <section className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm">
                    <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center"><BiBuildings size={20} /></div>
                        <input 
                            placeholder="UNIT REFERENCE (e.g. DUNCAN COURT A612)" 
            ...
            {/* RIGHT: THE CART (With Editable Prices) */}
            <div className="lg:col-span-1">
                <div className="bg-slate-900 text-white rounded-[2.5rem] p-8 shadow-2xl sticky top-8">
                    ...
                    <div className="space-y-3 mb-8 min-h-[200px] max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                        {cart.length === 0 && (
                    ...
                    <div className="space-y-3 pt-6 border-t border-slate-800">
                        ...
                        <div className="flex justify-between text-[10px] font-bold text-blue-400 uppercase tracking-widest">
                            <span>Mgmt Markup ({markup}%)</span>
                            <span>R{(calculateSubtotal() * markup / 100).toFixed(2)}</span>
                        </div>
                    ...
                    <button 
                        disabled={!unit || cart.length === 0}
                        onClick={createDraft}
                        className="w-full mt-8 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all active:scale-95 shadow-xl shadow-blue-500/20 flex items-center justify-center space-x-3"
                    >
                        ...
                        <div key={inv.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${
                                    ...
                                    'bg-orange-100 text-orange-700' // Draft
                                }`}>
                                    {inv.status}
                                </span>
                            </div>
      ...
      {/* --- REPORT DETAIL MODAL (THE BREAKDOWN) --- */}
      {viewingInvoice && (
                ...
                <div className="p-8 max-h-[50vh] overflow-y-auto">
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-100 space-y-2">
                        <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                           <span>Markup</span>
                           <span>{viewingInvoice.markup || 15}%</span>
                        </div>
                        <div className="flex justify-between text-xl font-black text-blue-600 uppercase">
                           <span>Total</span>
                           <span>R{viewingInvoice.total.toFixed(0)}</span>
                        </div>
                    </div>
                </div>
