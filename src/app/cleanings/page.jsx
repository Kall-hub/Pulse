...
// "Quick Add" Suggestions (Just helpers, not mandatory)
const SUGGESTIONS = {
...
const CleaningPage = () => {
  ...
  // STATE FOR INPUTS INSIDE THE MODAL (To type custom tasks)
  const [customInputs, setCustomInputs] = useState({}); 

  const [jobs, setJobs] = useState([
    {
      id: 1, unit: "DUNCAN COURT A612", cleaner: "Lindiwe", bookedOn: "11 JAN 2026", serviceDate: "2026-01-14", time: "09:00", status: "Booked",
      zones: [
        { 
            id: 'kit', name: 'Kitchen', 
            tasks: [
                { name: 'Oven Element', level: 'deep' },   
                { name: 'Counters', level: 'quick' }
            ] 
        }
      ]
    }
  ...
  const handleSave = (e) => {
    ...
    if (editingId) {
      ...
      setJobs([{ ...payload, id: Date.now(), bookedOn: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).toUpperCase(), status: 'Booked' }, ...jobs]);
    }
    closeModal();
  };

  // 1. SELECT ZONE (Starts Empty)
  ...
  const addTask = (zoneId, taskName) => {
    ...
    setFormData(prev => ({
        ...
        selectedZones: prev.selectedZones.map(z => {
            ...
            // Add with Default 'Standard' level
            return { ...z, tasks: [...z.tasks, { name: taskName, level: 'std' }] };
        })
    }));
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] text-slate-900 font-sans">
      ...
      <main className={`transition-all duration-300 ${isOpen ? "md:ml-64" : "md:ml-20"} ml-0 p-4 md:p-8`}>
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4 max-w-5xl mx-auto">
          <div>
          ...
          <button onClick={() => setIsModalOpen(true)} className="bg-slate-900 text-white py-4 px-8 rounded-2xl shadow-xl font-black text-[10px] uppercase tracking-widest flex items-center space-x-3 active:scale-95 border-b-4 border-blue-600">
          ...
          {filteredJobs.length === 0 ? (
            ...
            filteredJobs.map(job => (
              <CleaningCard 
                key={job.id} 
                {...job} 
                onDelete={() => { if(confirm("Delete?")) setJobs(jobs.filter(j => j.id !== job.id)) }}
                onEdit={() => startEdit(job)}
                onComplete={() => setJobs(jobs.map(j => j.id === job.id ? { ...j, status: 'Completed' } : j))}
                view={filter}
              />
            ))
          )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input required type="date" className="bg-slate-100 p-4 rounded-2xl text-[10px] font-black outline-none" value={formData.serviceDate} onChange={e => setFormData({...formData, serviceDate: e.target.value})} />
                <input required type="time" className="bg-slate-100 p-4 rounded-2xl text-[10px] font-black outline-none" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} />
              </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {Object.entries(ZONES).map(([key, config]) => {
                    ...
                    return (
                        ...
                        <span className="text-[8px] font-black uppercase">{config.name}</span>
                        </button>
                    )
                  })}
                </div>
              ...
              {formData.selectedZones.length > 0 && (
                  <div className="space-y-6">
                    ...
                    {formData.selectedZones.map((zone) => (
                        <div key={zone.id} className="bg-slate-50 border border-slate-200 p-5 rounded-3xl animate-in slide-in-from-top-2">
                            ...
                            {/* 1. INPUT AREA (ADD NEW) */}
                            <div className="mb-4">
                                <div className="flex gap-2 mb-3">
                                    <input 
                                        placeholder={`Add task for ${zone.name}...`}
                                        className="flex-1 bg-white p-3 rounded-xl text-[10px] font-bold outline-none border border-slate-200 focus:border-blue-500"
                                        value={customInputs[zone.id] || ''}
                                        onChange={(e) => setCustomInputs({...customInputs, [zone.id]: e.target.value})}
                                        onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addTask(zone.id, customInputs[zone.id]))}
                                    />
                                    <button 
                                        type="button"
                                        onClick={() => addTask(zone.id, customInputs[zone.id])}
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {SUGGESTIONS[zone.id].map(s => (
                                        <button 
                                            key={s} 
                                            type="button" 
                                            onClick={() => addTask(zone.id, s)}
                                            className="text-[8px] font-bold bg-white text-slate-400 px-2 py-1 rounded-lg border border-slate-200 hover:border-blue-300 hover:text-blue-500 transition-all flex items-center gap-1"
                                        >
                            ...
                            <div className="space-y-3">
                                {zone.tasks.length === 0 && <p className="text-[9px] text-slate-400 italic text-center py-2">No tasks added yet.</p>}
                                    ...
                                    <div key={task.name} className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-3 rounded-xl border border-slate-100 shadow-sm gap-3">
                                        ...
                                        <div className="flex gap-1">
                                            {Object.entries(LEVEL_CONFIG).map(([lvlKey, config]) => {
                                                const isActive = task.level === lvlKey;
                                                return (
                                                    <button 
                                                        key={lvlKey}
                                                        type="button"
                                                        onClick={() => setTaskLevel(zone.id, task.name, lvlKey)}
                                                        className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase transition-all flex items-center gap-1
                                                            ${isActive ? config.active : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}
                                                        `}
                                                    >
                                                        {config.icon}
...
/* CLEANING CARD */
const CleaningCard = ({ unit, cleaner, bookedOn, serviceDate, time, status, zones, onEdit, onDelete, onComplete, view }) => (
  <div className={`bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden transition-all animate-in fade-in slide-in-from-bottom-2 ${status === 'Completed' ? 'opacity-80 grayscale-[0.5]' : ''}`}>
    <div className="px-8 py-6 flex justify-between items-start bg-[#F8FAFC]">
      <div className="flex items-start space-x-5">
        <div className={`p-4 rounded-2xl shadow-lg text-white ${status === 'Completed' ? 'bg-slate-400' : 'bg-blue-600'}`}><FaSoap size={20} /></div>
        <div>
          <h2 className="text-xl font-black text-slate-900 tracking-tighter uppercase italic leading-none mb-2">{unit}</h2>
    ...
    <div className="p-6 space-y-3">
        {zones.map((zone, i) => (
            <div key={i} className="flex flex-col md:flex-row justify-between p-4 rounded-2xl border border-slate-100 bg-white shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
                <div className="flex items-center space-x-3 mb-3 md:mb-0">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 text-lg">
                        {ZONES[zone.id]?.icon || <FaSoap/>}
                    </div>
                    <span className="text-[10px] font-black uppercase text-slate-900">{zone.name}</span>
                </div>
                <div className="flex flex-wrap gap-2 justify-end max-w-[350px]">
                    {zone.tasks.map((task, idx) => {
                        const lvl = LEVEL_CONFIG[task.level];
                        return (
                            <div key={idx} className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border ${lvl.color}`}>
                                <span className="text-[7px]">{lvl.icon}</span>
                                <span className="text-[8px] font-bold uppercase">{task.name} ({lvl.label})</span>
                            </div>
                        )
                    })}
                </div>
